...
adobereaderdc-update)
    ...
    if [[ -d "/Applications/Adobe Acrobat Reader.app" ]]; then
        printlog "Found /Applications/Adobe Acrobat Reader.app"
        mkdir -p "/Library/Application Support/Adobe/Acrobat/11.0"
        defaults write "/Library/Application Support/Adobe/Acrobat/11.0/com.adobe.Acrobat.InstallerOverrides.plist" ReaderAppPath "/Applications/Adobe Acrobat Reader.app"
        if ! defaults read "/Applications/Adobe Acrobat Reader.app/Contents/Resources/AcroLocale.plist" ; then
            printlog "Missing locale data, this will cause the updater to fail. Deleting Adobe Acrobat Reader.app and installing fresh." WARN
...
googledrivefilestream)
     # credit: Isaac Ordonez, Mann consulting (@mannconsulting)
    # version credit: Eddie Vartanessian
    ...
    version=$(curl -fs https://support.google.com/a/answer/7577057\?hl\=en | grep "Version " | sed '1p;d;/Version /!d' | awk -F' ' '{print $5}' | cut -b 1-2)
    newVersion=$(($version - 1))
    appNewVersion="$newVersion.0.0.0"
    expectedTeamID="EQHXZ8M8AV"
    ;;
...
microsoftexcel)
    ...
    appNewVersion=$(curl -fsIL "$downloadURL" | grep -i location: | grep -o "/Microsoft_.*pkg" | cut -d "_" -f 3 | cut -d "." -f 1-3)
    versionKey="CFBundleVersion"
    expectedTeamID="UBF8T346G9"
    # if [[ -x "/Library/Application Support/Microsoft/MAU2.0/Microsoft AutoUpdate.app/Contents/MacOS/msupdate" && $INSTALL != "force" && $DEBUG -eq 0 ]]; then
    #     printlog "Running msupdate --list"
...
scaleft)
    name="ScaleFT"
    type="pkg"
    appNewVersion=$(curl -fs "https://dist.scaleft.com/repos/macos/stable/all/macos-client/" | sed -n '/<li>/p' | tail -n 1 | awk -F'[<v>]' '{print $7}')
    downloadURL="https://dist.scaleft.com/repos/macos/stable/all/macos-client/v${appNewVersion}/ScaleFT-${appNewVersion}.pkg"
...
wireshark)
    name="Wireshark"
    type="dmg"
    wiresharkDownloadPageHTML="$(curl -sfL 'https://www.wireshark.org/download.html')"
    appNewVersion="$(echo -E "${wiresharkDownloadPageHTML}" | xmllint --html --xpath 'normalize-space(substring-after(//summary[starts-with(text(),"Stable Release:")]/text(),":"))' - 2> /dev/null)"
    if [[ $(arch) == i386 ]]; then
      # downloadURL for Intel:
      downloadURL="$(echo -E "${wiresharkDownloadPageHTML}" | xmllint --html --xpath "string(//a[contains(@href,'${appNewVersion}%20Intel%2064.dmg')]/@href)" - 2> /dev/null)"
    elif [[ $(arch) == arm64 ]]; then
      # downloadURL for Apple Silicon:
      downloadURL="$(echo -E "${wiresharkDownloadPageHTML}" | xmllint --html --xpath "string(//a[contains(@href,'${appNewVersion}%20Arm%2064.dmg')]/@href)" - 2> /dev/null)"
    fi
